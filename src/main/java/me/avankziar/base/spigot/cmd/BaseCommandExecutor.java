package main.java.me.avankziar.base.spigot.cmd;
...
import main.java.me.avankziar.base.general.ChatApi;
import main.java.me.avankziar.base.spigot.BaseMain;
import main.java.me.avankziar.base.spigot.assistance.MatchApi;
import main.java.me.avankziar.base.spigot.cmdtree.ArgumentConstructor;
...
public class BaseCommandExecutor implements CommandExecutor
{
	...
	public BaseCommandExecutor(BaseMain plugin, CommandConstructor cc)
	{
		this.plugin = plugin;
	...
	public boolean onCommand(CommandSender sender, Command cmd, String lable, String[] args) 
	{
		...
		{
			BaseMain.log.info("/%cmd% is only for Player!".replace("%cmd%", cc.getName()));
			return false;
		}
				...
				if(args[i].equalsIgnoreCase(ac.getName()))
				{
					if(length >= ac.minArgsConstructor && length <= ac.maxArgsConstructor)
					{
						if(player.hasPermission(ac.getPermission()))
						{
							...
							{
								BaseMain.log.info("ArgumentModule from ArgumentConstructor %ac% not found! ERROR!"
		...
		player.spigot().sendMessage(ChatApi.clickEvent(plugin.getYamlHandler().getLang().getString("InputIsWrong"),
				ClickEvent.Action.RUN_COMMAND, BaseMain.infoCommand));
		return false;
	}
	...
	public void baseCommands(final Player player, int page)
	{
		...
		player.sendMessage(ChatApi.tl(plugin.getYamlHandler().getLang().getString(
				BaseMain.infoCommandPath+".Headline")));
		...
		pastNextPage(player, BaseMain.infoCommandPath, page, lastpage, BaseMain.infoCommand);
	}
